...
def train(params:argparse.Namespace):
    ...
    if params.dataset == 'cifar10':
        from dataloader_cifar import load_data2, transback
        dataloader, sampler = load_data2(params.clean_data_path, params.poison_data_path)
    # elif params.dataset == 'imagenette':
    #     from dataloader_imagenette import load_data2, transback
    #     dataloader, sampler = load_data2(params.clean_data_path, params.poison_data_path)
    else:
    ...
    if os.path.exists(lastpath):
        print(f'resuming from {lastpath}')
        ...
        if (epc + 1) % params.interval == 0:
            ...
            with torch.no_grad():
                ...
                * torch.arange(start = 0, end = params.clsnum).reshape(-1, 1)
                ...
                if local_rank == 0:
                    os.makedirs(os.path.join(params.samdir), exist_ok=True)
            ...
            os.makedirs(os.path.join(params.moddir), exist_ok=True)
...
def main():
    ...
    parser.add_argument('--samdir',type=str,default='train_sample',help='sample addresses')
    ...
    parser.add_argument("--clean_data_path", type=str, default='../data/CIFAR10/cifar10')
    parser.add_argument("--poison_data_path", type=str, default='../data/CIFAR10/poison/badnet_pr5_pr0.1_pt4')
    # parser.add_argument('--save_data', default=False, action=argparse.BooleanOptionalAction)

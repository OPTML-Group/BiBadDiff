...
# def open_cifar10_badnet(tarball: str, *, max_images: Optional[int], poison_label, poison_rate):
#     images = []
#     labels = []
...
#     idx = np.where(labels != poison_label)[0]
#     poison_idx = np.random.choice(idx, size=int(len(images) * poison_rate), replace=False)
#     images[poison_idx, -3:-1, -3:-1, :] = 0
#     labels[poison_idx] = poison_label
...
def open_cifar10_custom(source: str, *, max_images: Optional[int]):
    # images = []
    # labels = []
...
def open_npz(source: str, *, max_images: Optional[int]):
    # images = []
    # labels = []
...
def open_dataset(source, *, max_images: Optional[int], backdoor):
...
def main(
    ...
    # num_files, input_iter = open_cifar10_badnet(source, max_images=max_images, 
    #             poison_label=0, poison_rate=0.01)
    # num_files, input_iter = open_cifar10_custom(source, max_images=max_images)
    num_files, input_iter = open_npz(source, max_images=max_images)
